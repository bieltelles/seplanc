...
export async function GET(request: NextRequest) {
  try {
    ...
    const periodos = await getRgfQuadrimestresDisponiveis(ano);

    if (!quadrimestre) {
      return NextResponse.json({ anos, periodos, data: null });
    }

    const anexos = (await getRgfAnexos(ano, quadrimestre, entidade)).map((a) => a.anexo);

    if (!anexo) {
      return NextResponse.json({ anos, periodos, anexos, data: null });
    }

    const data = await getRgfData(ano, quadrimestre, entidade, anexo);
